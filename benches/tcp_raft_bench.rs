...
impl RaftStorage for MemStorage {
    ...
    fn save_snapshot(&self, _meta: &arbitro_raft::SnapshotMeta, _snapshot: &[u8]) -> Result<(), RaftError> { Ok(()) }
    fn load_snapshot(&self) -> Result<Option<(arbitro_raft::SnapshotMeta, Vec<u8>)>, RaftError> { Ok(None) }
}
...
struct TcpTransportInner {
    ...
    inbound_tx: futures::channel::mpsc::UnboundedSender<InboundRaftMessageView>,
    inbound_rx: tokio::sync::Mutex<futures::channel::mpsc::UnboundedReceiver<InboundRaftMessageView>>,
        ...
        tokio::spawn(async move {
            while let Ok((mut stream, _)) = listener.accept().await {
                ...
                tokio::spawn(async move {
                    ...
                    loop {
                        if stream.read_buf(&mut buf).await.is_err() { break; }
                        
                        while buf.len() >= 32 {
                            let body_len = u32::from_le_bytes(buf[24..28].try_into().unwrap()) as usize;
                            let total_len = 32 + body_len;
                            if buf.len() < total_len { break; }
                            
                            let frame = buf.split_to(total_len).freeze();
                            if let Ok(view) = decode_message_view(frame) {
...
impl RaftTransport for TcpTransport {
    async fn send(&self, peer: PeerId, msg: RaftMessage) -> Result<(), RaftError> {
        let addr = self.inner.peer_addrs.get(&peer).copied()
        ...
        encode_message_into(self.inner.local_id, &msg, &mut buf)?;
        stream.write_all(&buf).await.map_err(|e| RaftError::Transport(e.to_string()))?;
        Ok(())
    }
...
fn bench_tcp_raft(c: &mut Criterion) {
    ...
    for clients in [1, 1024] {
        ...
        group.bench_with_input(BenchmarkId::new("single_writes_tcp", clients), &clients, |b, &clients| {
            ...
            b.to_async(&rt).iter_custom(|iters| {
                async move {
                    ...
                    let raft_task = tokio::spawn(async move { let _ = raft1.run().await; });
