    ...
    fn read_entries<'a>(
        ...
        for e in entries.iter() {
            if e.index >= from && e.index < to {
                ...
                // SAFETY: payload_buf lives for 'a.
...
#[ignore = "runtime-fragile: relies on tokio scheduling between the manually-\
            driven leader and spawned followers; compilation is the primary \
            acceptance gate. Un-ignore once the harness stabilizes."]
...
#[tokio::test(flavor = "multi_thread")]
#[ignore = "runtime-fragile: post-snapshot AppendEntries reconciliation depends \
            on the follower's log_metadata accepting prev_log_index at the \
            snapshot boundary; verify manually once the pipeline is deterministic."]
